...
export function KanbanCardHeader({ itemId, badge, badgeColor, onEdit, onDelete }: KanbanCardHeaderProps) {
  return (
      ...
      <div className="flex -space-x-2">
        {assignees?.slice(0, 3).map((a, i) => (
          <Avatar key={i} className="size-6 border-2 border-background">
            ...
            <AvatarFallback className="text-[9px]">{a.name.slice(0, 2).toUpperCase()}</AvatarFallback>
          </Avatar>
        ))}
      </div>
