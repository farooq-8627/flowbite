...
| 8 | **First-time tour for `<ChatLandingPane>`** uses `<FirstTimeTour id="chat-panel-v1">` with 3 steps tagged via `data-tour="landing-pulse" / "landing-actions" / "landing-chips"`, and is GATED on a `tourEnabled` prop so it fires only when the chat panel is actually open. The desktop panel stays mounted off-screen when "closed"; `tourEnabled` is drilled `DashboardLayoutClient.chatOpen → AIChatPanel/AIChatPanelContent → ChatSheet → ChatLandingPane` so the coachmark never points at off-screen anchors. Bumped from `chat-landing-v1` → `chat-panel-v1` on the 2026-06-06 tour overhaul. | Per `rules.md §2.6` — power gestures get a one-time coachmark, never a tooltip; and a coachmark must never fire against a hidden surface. |
...
| 12 | **`<AISuggestionsPanel>` reads `api.ai.suggestions:list`**, not LLM calls. Pure heuristics over already-cached data so the panel runs on every render without latency or cost. **As of 2026-06-06 the ORG-scope panel was folded into `<AIPulseRibbon>`** — the dashboard now has ONE proactive surface, not two; `AISuggestionsPanel` survives only for ENTITY scope (profile/deal/company detail). `ai.suggestions:list` is now permission-gated **per category** (overdue→`tasks.view`, stale leads→`leads.view`, stuck deals→`deals.view`; entity scope by entity type) — no blanket `leads.view` throw — and the ranked store (`nextActions:listForUser`) filters rows by record-kind→view-perm (`resolveAllowedKinds`). Empty/all-clear states still hand off to chat via `intent` prefill. | Latency-free proactive surface; the model never sees the suggestion list. One merged, permission-scoped surface ends the "two panels showing the same thing" confusion and never leaks a category the role can't view. |
