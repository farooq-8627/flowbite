...
function parseColumnFilterValue(value: unknown) {
  ...
  if (Array.isArray(value)) return value.map((i) => (typeof i === "number" || typeof i === "string" ? i : undefined));
...
export function DataTableDateFilter<TData>({ column, title, multiple }: DataTableDateFilterProps<TData>) {
  ...
  const onSelect = React.useCallback(
    (date: Date | DateRange | undefined) => {
      if (!date) { column.setFilterValue(undefined); return; }
    ...
    if (multiple) {
      ...
      return (
        <span className="flex items-center gap-2">
          <span>{title}</span>
          {has && <><Separator orientation="vertical" className="mx-0.5 data-[orientation=vertical]:h-4" /><span>{text}</span></>}
        </span>
      );
    }
    if (getIsDateRange(selectedDates)) return null;
