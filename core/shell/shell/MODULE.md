...
| D11 | (2026-05-29 hotfix wave) `lazyWarmForUser` per-`(userId,orgId)` rate budget raised 1/min → 5/min and the handler now soft-fails on rate-limit (returns `{ scheduled: false, rateLimited: true }`) instead of throwing. AIPulseRibbon also gates the call on a sessionStorage TTL keyed `orbitly:ai:lazyWarm:${orgId}:${userId}` (60s). | Original 1/min budget was set defensively when the warm was a manual click only. With AIPulseRibbon's auto-warm-on-empty wired to the dashboard, every navigation back to `/` remounts the component and tries to warm — within 60s of a successful warm the second call would `throw ConvexError("Too many requests")`, which is the storm the user pasted from the Convex logs. The sessionStorage gate prevents the storm at the source; the budget bump tolerates fast multi-tab use; the soft-fail return type means the Convex error log no longer records benign rate-limit drops. The actual rebuild is bounded by `internal.rebuildForUser` running on the scheduler and by `nextActionsTrigger.ts`'s 5s token-bucket dedup, so loosening the public-mutation budget does not change the effective backend pressure. |
...
| D21 | (2026-06-06) The dashboard has ONE proactive surface: `<AIPulseRibbon>`. The standalone org-scope `<AISuggestionsPanel>` was removed from `DashboardHomeView`; the pulse now merges the per-user ranked store (`nextActions:listForUser`) with the org-wide heuristic (`ai.suggestions:list`, scope "org"), deduped (anchored heuristic suggestion dropped when a ranked row covers the same record; anchorless aggregates like "N leads untouched" kept). Both sources are permission-gated **per category** server-side (`resolveAllowedKinds` for the ranked store; per-block gates in `suggestForOrg`) — a role only ever sees `leads`/`deals`/`tasks`/`contacts`/`companies` signals it can view, and a role with none sees an empty surface (no throw). The pulse header is its own collapse toggle, persisted in `dashboardSectionsCollapsed.aiPulse`, independent of the cockpit's `proactive` key. `AISuggestionsPanel` is retained for ENTITY scope (profile/deal/company detail). | The user had two panels showing roughly the same overdue follow-ups but diverging on org-wide signals (Pulse = assigned-to-me only; suggestions = whole org but gated solely on `leads.view`). Merging removes the duplication and the "shows the same but not all" confusion; per-category gating answers the explicit ask — "whatever the role has permission that should be shown, not full org data". Keeping the Pulse UI (the user preferred it) and giving it the cockpit's collapse pattern at a finer grain lets users fold just the pulse without folding the whole AI cluster. |
