...
> Last Updated: 2026-04-21 | Author: Base Agent
...
### 📝 Notes (Phase 2 — NOT YET IMPLEMENTED)

> **Status:** NOT IMPLEMENTED — build in Phase 2 alongside CRM core.
> Notes are short-form text attached to any CRM entity. Can be created by users or AI.

| Action | owner | admin | member | viewer | client | partner |
|---|---|---|---|---|---|---|
| View non-internal notes | ✅ | ✅ | ✅ | 👁️ | own entity | own entity |
| View internal notes | ✅ | ✅ | ✅ | 👁️ | ❌ | ❌ |
| Create note | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| Edit own note | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| Edit any note | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| Delete note | ✅ | ✅ | own | ❌ | ❌ | ❌ |
| Pin note | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| AI: create note | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| AI: search notes | ✅ | ✅ | ✅ | 👁️ | own entity | own entity |

> **Internal notes** (`isInternal: true`) are never shown to client/partner portal users.
> AI-created notes always have `authorType: "ai"` and appear with a distinct badge in the timeline.
...
| View deals in company | ✅ | ✅ | ✅ | 👁️ | ❌ | ❌ |
| AI: `searchCompanies` | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ |
| AI: `createCompany` | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| AI: `getCompanyContacts` | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ |
| AI: `getCompanyDeals` | ✅ | ✅ | ✅ | 👁️ | ❌ | ❌ |
| AI: `getCompanySummary` | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ |

> Companies are internal-only entities. Clients and partners do not have access to the company structure.
...
### 🤖 AI Assistant (Phase 3 — NOT YET IMPLEMENTED)

> **Status:** NOT IMPLEMENTED — build in Phase 3.
> AI access is role-scoped. Different roles get different tool sets.
> Tool availability is checked BEFORE the Claude API call (tools not in the role's set are never sent to Claude).

| Action | owner | admin | member | viewer | client | partner |
|---|---|---|---|---|---|---|
| Use AI assistant (chat) | ✅ | ✅ | ✅ | ✅ | ✅ (scoped) | ✅ (scoped) |
| AI: search records | ✅ | ✅ | ✅ | 👁️ | own | own |
| AI: create records | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| AI: update records | ✅ | ✅ | own | ❌ | ❌ | ❌ |
| AI: delete records | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| AI: bulk update | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| AI: view pipeline analytics | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| AI: view team performance | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| AI: view deal values | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| AI: draft emails | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| AI: set reminders | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| AI: view project status | ✅ | ✅ | ✅ | 👁️ | own | own |
| AI: view conversation history | own | own | own | own | own | own |
| AI: view org-wide AI logs | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |

#### AI Tool Sets by Role

| Role | Available Tool Categories |
|---|---|
| **owner/admin** | ALL tools: search, create, update, delete, bulk_update, analytics, pipeline, team_performance, forecast, reminders, email_draft, project_status |
| **member** | search, create, update_own, detail, reminders, email_draft, project_status, notes |
| **viewer** | search (read-only), detail (read-only), project_status (read-only) |
| **client** | search_own_connections, project_status (own), submit_request, view_deliverables |
| **partner** | search_own_connections, project_status (own), submit_response, view_assignments |

#### AI Data Filtering Rules

1. **client role**: Cannot see deal values, internal notes, other clients' connections, team performance data
2. **partner role**: Cannot see deal values, internal notes, other partners' connections, billing info
3. **viewer role**: Cannot see any fields marked as `sensitive` in fieldDefinitions
4. **member role**: Can see all non-admin data; cannot see org-wide analytics or other members' private notes

#### AI Disambiguation Rules

| Search Results | AI Behavior | UI Element |
|---|---|---|
| 0 results | "Not found. Want me to create one?" | Text response |
| 1 result | Proceed immediately (no confirmation) | Auto-continue |
| 2–10 results | Show all as clickable record cards | `ChatToolCall` cards |
| 10+ results | Ask user to narrow down (email, company, etc.) | Text response |

#### AI Confirmation Rules (Destructive Actions)

| Action Type | Requires Confirmation? | Confirmation UI |
|---|---|---|
| Search / detail / list | ❌ No | Auto-execute |
| Update status / add note | ❌ No | Auto-execute |
| Set reminder | ❌ No | Auto-execute |
| Send email | ✅ YES | Email preview card + [Send] / [Edit] |
| Delete record | ✅ YES | Record name + [Confirm Delete] / [Cancel] |
| Bulk update (2+ records) | ✅ YES | Count + action preview + [Confirm] / [Cancel] |
| Irreversible stage change | ✅ YES | Stage name + affected records + [Confirm] / [Cancel] |
...
```
free:        crm.basic (max 50 contacts, 1 pipeline), ai.trial (10 AI messages/day)
starter:     crm.full, ai.basic (500 messages/month), reports.basic, portal.basic
pro:         crm.full, ai.full (unlimited), integrations.basic, reports.full, portal.full
enterprise:  pro + integrations.full, custom_branding, api_access, audit_logs.full, white_label
```
...
| `core/shell/hooks/useOrgPermission.ts` | React hook for frontend permission checks |
