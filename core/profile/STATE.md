...
- **Names of groups are human-fixed here** (Overview, Messages, Timeline, …). We don't run these through `useEntityLabels()` because they describe relational concepts that don't change when an org renames entities. If any ever *should* be dynamic (e.g. "Deals" tab when org renamed Deals → Opportunities), we'll wire that one through `useEntityLabels()` in the group definition.
