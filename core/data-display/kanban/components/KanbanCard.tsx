...
export function KanbanCard({ itemId, isDragging, onClick, children, className }: KanbanCardProps) {
	return (
		<KanbanItem value={itemId} asChild>
			<Card
				className={cn(
					// Base look — hover keeps a subtle shadow when idle.
					"my-2 w-64 md:w-72 cursor-pointer transition-shadow",
					!isDragging && "hover:shadow-md",
					// While dragging the overlay clone renders this card. We want
					// to see the FULL card with no opacity, no rotation, and no
					// drop-shadow halo so the movement feels native.
