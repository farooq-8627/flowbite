  ...
  return (
    <AuthShellLayout
      panel={{
        ...
        bottomLeft: { heading: "Close deals faster", body: "Track every lead, contact, and opportunity in one place." },
        bottomRight: { heading: "Need help?", body: "Our support team is available 24/7 — just reach out." },
      }}
    >
      <div className="absolute top-5 end-0 flex w-full justify-end px-10">
        <p className="text-muted-foreground text-sm">
          Don&apos;t have an account?{" "}
          <Link prefetch={false} className="text-foreground font-medium underline underline-offset-4" href="/signup">
            Register
          </Link>
        </p>
      </div>
          ...
          <form
            ...
            onSubmit={(e) => {
              ...
              void signIn("password", formData)
                ...
                .catch((err: Error) => {
                  posthog.capture("sign_in_failed", { error_message: err.message });
                  toast.authError(err);
                  setLoading(false);
                });
            }}
            ...
            <FieldGroup className="gap-4">
              ...
              <Field className="gap-1.5">
                ...
                <div className="flex items-center justify-between">
                  <FieldLabel htmlFor="signin-password" className="sr-only">Password</FieldLabel>
                  <Link prefetch={false} className="ms-auto text-muted-foreground text-xs underline underline-offset-4 hover:text-foreground" href="/forgot-password">
                    Forgot password?
                  </Link>
                </div>
                <Input id="signin-password" name="password" type="password" placeholder="••••••••" autoComplete="current-password" required />
              </Field>
              <Field orientation="horizontal">
                <Checkbox id="signin-remember" checked={remember} onCheckedChange={(v) => setRemember(Boolean(v))} />
                <FieldContent>
                  <FieldLabel htmlFor="signin-remember" className="font-normal">Remember me for 30 days</FieldLabel>
                </FieldContent>
              </Field>
            </FieldGroup>
