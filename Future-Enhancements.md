...
## B.45, ✅ SHIPPED 2026-06-07 — Per-user dashboard aggregate counter scoping

`getDashboardStats` (`convex/orgs/queries.ts`) now branches on `resolveRecordScope`. Members WITH `records.viewAll` use the unchanged O(1) fast path via `readAllOrgStats`. Members WITHOUT walk `by_org_and_assignee` per entity (leads/contacts/companies/deals — bounded `take(500)`), counting non-soft-deleted rows and summing the open-deal `value` for pipelineValue. Tasks dueToday/overdue/done apply an `assignedTo === userId` filter. Recent activity surfaces rows where the actor is the caller OR `personCode` matches one of their assigned leads/contacts. Member count stays org-wide. New `scoped: boolean` field on the return shape so the UI can render a "Showing your assigned records only" hint. See `SHIPPED.md` 2026-06-07 entry "Full-dynamic AI tooling pass".

**Still deferred** (separate, lower-impact follow-ups — leak counts/secondary names, not primary record rows):
- **Org-wide activity timeline** — `activityLogs` rows don't carry `assignedTo`; scoping needs a per-row resolve back to the referenced record's current assignee (or denormalising assignee onto the log).
- **Company-membership join/picker helpers** (`convex/crm/entities/companies/queries.ts`: `getByPersonCode`, `listPersonsForCompany`, `listAllPersonsWithCompany`, `listPersonsWithoutCompany`, `listCompaniesByPersonCodes`) — surface person rosters; scoping risks breaking the company-association UX for a scoped member managing a company. Needs a UX decision on "roster of a company I'm assigned to" vs strict per-row.

---

## B.47, Propagate `customFields` through contact / company / deal `createForAI` (bulk_create_entities full parity)

| Field           | Value                                                                              |
|-----------------|------------------------------------------------------------------------------------|
| Status          | Backlog                                                                            |
| Category        | AI tooling / data fidelity                                                         |
| Phase to ship   | When a real customer asks "create N companies with full data" / contacts / deals via AI |
| Owners          | `convex/crm/entities/{contacts,companies,deals}/mutations.ts` (createForAI + createImpl) |
| Risk if skipped | Low — leads (the highest-frequency bulk-create target) already get full custom-field propagation. Contact/company/deal bulk-creates today land core columns + drop top-level custom fields with a per-row warning. The model OR user can follow up with `update_entity` per row to fill the rest. |
| Files involved  | `convex/crm/entities/contacts/mutations.ts`, `convex/crm/entities/companies/mutations.ts`, `convex/crm/entities/deals/mutations.ts`, `convex/crm/shared/bulk/capabilities.ts::buildCreateArgs` (each entity branch needs `customFields` spread once the underlying createForAI accepts it) |

**Why we deferred:** The 2026-06-06 inline-approval + bulk-data-fidelity ship landed `partitionRowKeys` for ALL FOUR entity types (lead/contact/company/deal). Lead's `createForAI` already accepts `customFields` end-to-end (audit Finding #3, 2026-06-05) so leads got full parity. Contact/company/deal's `*ForAI` paths still don't accept `customFields` — extending each requires: (a) widening the createForAI validator with `customFields: v.optional(v.record(v.string(), v.any()))`, (b) inlining `applyCustomFieldsForRecordImpl` into each entity's `createImpl` (mirroring the leads inline path), (c) updating bulk's `buildCreateArgs` per-entity branches to spread `customFields`, (d) tests. Estimated 2-3 hours per entity, ~half a day total. Not in scope for the regression-fix session that landed B.

**Benefits when reinstated:** Bulk-create works at full data fidelity for every entity type. A turn like "create 10 sample companies with full data" or "create 5 deals across Sales pipeline with custom fields filled" lands every column in one round-trip instead of needing per-row `update_entity` follow-ups.

**Use cases / who it protects:** AI-driven seed data flows that span contact/company/deal entities (currently leads-only via `bulk_create_entities`). Direct CSV-shaped paste flows for non-lead entities.

**Implementation sketch (per entity, mirroring leads):**
1. Widen `createForAI` validator: `customFields: v.optional(v.record(v.string(), v.any()))`.
2. Extend `createImpl` arg type with `customFields?: Record<string, unknown>`; after the entity insert + activity log, call `applyCustomFieldsForRecordImpl(ctx, { orgId, userId, entityType, entityId, customFields })` if the map is present.
3. Return `appliedCustomFields` + `unknownFields` in the createImpl result so callers can surface them.
4. Update `convex/crm/shared/bulk/capabilities.ts::buildCreateArgs` for that entity to spread `customFields` from the row partition's `customFields` bucket (currently lead-only).
5. New tests in the entity's mutations test file (mirror `leads.createWithCustomFields` test).
6. Drop the "no customFields propagation today" caveat from the bulk group playbook + `bulkRowPartition.ts`.

**Verification:** Run `bulk_create_entities` with `entityType: "company"` (or contact / deal) + a row carrying both column args + a `customFields:{}` map; confirm via Convex dashboard that the resulting row has `fieldValues` rows for the org-defined custom fields, not just the columns.

---

## B.46, Step-up confirmation hardening: frontend test + argsHash robustness for non-passthrough caps

| Field           | Value                                                                              |
|-----------------|------------------------------------------------------------------------------------|
| Status          | Backlog (follow-up to the 2026-06-06 step-up-card-render fix)                       |
| Category        | AI tooling / 2FA step-up                                                            |
| Phase to ship   | Next AI hardening pass                                                              |
| Owners          | `core/ai/components/AssistantTurn.tsx`, `core/ai/components/StepUpCard.tsx`, `convex/aiStepUp.ts`, `convex/ai/registry/wrapper.ts` |
| Risk if skipped | Low. The render fix is covered by manual repro + the full convex suite; the argsHash edge only bites capabilities whose zod schema transforms input. |

**Two gaps left after the 2026-06-06 fix:**

1. **No frontend test for step-up card detection.** `findStepUpRequest` now scans the grouped `role:"tool"` rows (regression-prone, it broke once already by reading `assistant.toolCalls`). Add a `vitest.config.ts` (frontend) test that feeds a grouped turn (`assistant` + a `tool` row whose `toolCalls[0].output.status === "needs_step_up"`) and asserts the `<StepUpCard>` mounts, plus a negative case (envelope on a non-last turn → no card). No frontend test currently imports these components.

2. **argsHash reproduction is only robust for passthrough schemas.** `confirmStepUp` hashes the RAW card args (the persisted tool `input`); the wrapper's verifier hashes the PARSED+resolved args of the re-run. For `bulk_create_entities` these are equal (the `rows` schema is `z.record(z.string(), z.unknown())` passthrough, no top-level defaults), so embedding the verbatim args in the synthetic message (shipped) makes the hash match. But a capability whose zod schema applies `.default()` / `z.preprocess` / ref-resolution would canonicalise differently between the two hash sites → token mismatch → confirm loop, even with verbatim args. Robust fix options: (a) hash a canonical PARSED form on both sides, or (b) have `confirmStepUp` persist the confirmed args server-side and re-run the capability directly (bypassing the model re-call), bigger change, revisit if a non-passthrough irreversible cap ever needs step-up. Today the only step-up caps are the `bulk`/destructive group (passthrough or id-list schemas), so this is latent, not active.

**Verification when picked up:** frontend test green for both card cases; a synthetic test that round-trips `confirmStepUp` → re-run for a defaulted-schema cap and asserts the token verifies.
...
| 2026-06-06 | RBAC          | B.45, Extend `records.viewAll` row-level scope to dashboard aggregates, org timeline, and company-join/picker helpers (✅ dashboard aggregates SHIPPED 2026-06-07; timeline + company-join still deferred) | B |
| 2026-06-07 | AI tooling    | Full-dynamic AI tooling pass: `create_deal` pipelineName+stageName, dynamic entityType validation across 11 capabilities, per-org taskTypes, dynamic WhatsApp template hint, dynamic Default-Pipeline copy, B.45 dashboard scope (SHIPPED) | — |
| 2026-06-06 | AI tooling    | B.47, Propagate `customFields` through contact / company / deal `createForAI` (leads already done; covers bulk_create full parity) | B |
