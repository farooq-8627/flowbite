...
| 28 | **Row-level record visibility = the single `records.viewAll` capability, gated PER ROLE** (locked 2026-06-06). It's a CAPABILITY (presence grants full visibility, absence restricts to `assignedTo === own userId`); ONE cross-cutting key gates leads/contacts/companies/deals together, never per-entity. Seeded to all 4 system roles + backfilled onto every existing role (`_migrations/2026_06_06_backfillRecordsViewAll`) so behaviour is unchanged by default; an owner opts a role into assigned-only by REMOVING the key in the role editor (the role IS the unit of control, no org-wide mode flag). Enforced ONLY through `convex/_shared/permissions/recordScope.ts` (`resolveRecordScope` / `rowInScope` / `resolveAssigneeFilter` / `scopeAssignee`) on list/board/detail/search (+ `*ForAI` twins → AI `search_crm` inherits it). The AI Pulse is already per-user assigned-scoped. Deferred surfaces (dashboard aggregates, org timeline, company-join helpers) tracked in `Future-Enhancements.md §B.45`. |
