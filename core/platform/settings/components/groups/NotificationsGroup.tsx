...
type NotifGroupDef = {
	id: string;
	title: string;
	/** String or a function of labels — lets the CRM group's blurb mirror admin renames. */
	description: string | ((l: ReturnType<typeof resolveEntityLabels>) => string);
	items: NotifItem[];
};
...
const NOTIF_GROUPS: NotifGroupDef[] = [
	{
		...
		title: "CRM",
		description: (l) =>
			`Notifications for ${l.lead.singular.toLowerCase()}, ${l.contact.singular.toLowerCase()}, and ${l.deal.singular.toLowerCase()} activity.`,
		items: [
			{
				key: "lead_assigned",
				label: (l) => `${l.lead.singular} assigned`,
				description: "When a lead is assigned to you.",
			},
			{
				key: "lead_converted",
				label: (l) => `${l.lead.singular} converted`,
				description: "When a lead is converted to a contact.",
			},
			{
				key: "contact_assigned",
				label: (l) => `${l.contact.singular} assigned`,
				description: "When a contact is assigned to you.",
			},
			{
				key: "deal_assigned",
				label: (l) => `${l.deal.singular} assigned`,
				description: "When a deal is assigned to you.",
			},
			{
				key: "deal_stage_changed",
				label: (l) => `${l.deal.singular} stage changed`,
				description: "When a deal moves to a new stage.",
			},
			{
				key: "deal_won",
				label: (l) => `${l.deal.singular} won`,
				description: "When a deal is marked as won.",
			},
			{
				key: "deal_stale",
				label: (l) => `${l.deal.singular} gone stale`,
				description: "When a deal has had no activity.",
			},
		],
	},
	{
		...
		items: [
			{
				...
				description: "When an AI action finishes running.",
			},
			{
				key: "ai_workspace_setup",
				label: () => "AI workspace setup",
				description: "When AI workspace setup is complete.",
			},
		],
	},
	{
		...
		items: [
			...
			},
			{
				...
				description: "When your role is updated by an admin.",
			},
		],
	},
	{
		...
		items: [
			{
				...
				description: "When your trial is about to expire.",
			},
			{
			...
			},
			{
				...
				description: "When a CSV import finishes successfully.",
			},
			{
				key: "csv_import_failed",
				label: () => "CSV import failed",
				description: "When a CSV import encounters an error.",
			},
		],
	},
];
...
function NotifGroupCard({
	...
	// Descriptions can be either a plain string or a labels-aware function —
	// the CRM group uses the function form so renames propagate instantly.
	const description =
		typeof group.description === "function" ? group.description(labels) : group.description;
...
export function NotificationsGroup({ org }: { org: OrgSettings }) {
	...
	const handleToggleAll = async (group: NotifGroupDef, value: boolean) => {
		const patch = Object.fromEntries(group.items.map((i) => [i.key, value])) as Parameters<
			typeof updatePrefs
		>[0]["preferences"];
		try {
